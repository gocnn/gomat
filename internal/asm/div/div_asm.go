...
//go:generate go run . -bits 64 -out ../../../asm/asm64/div_amd64.s -stubs ../../../asm/asm64/div_amd64.go -pkg asm64
//go:generate go run . -bits 32 -out ../../../asm/asm32/div_amd64.s -stubs ../../../asm/asm32/div_amd64.go -pkg asm32
...
var (
	DIVP   = map[int]func(operand.Op, operand.Op){32: build.DIVPS, 64: build.DIVPD}
	DIVS   = map[int]func(operand.Op, operand.Op){32: build.DIVSS, 64: build.DIVSD}
	...
	VDIVP  = map[int]func(...operand.Op){32: build.VDIVPS, 64: build.VDIVPD}
...
func buildAVX(bits int) {
	...
	build.Doc(fmt.Sprintf("%s divides x1 by x2, element-wise, storing the result in y (%d bits, AVX required).", name, bits))
...
func buildSSE(bits int) {
	...
	build.TEXT(name, build.NOSPLIT, signature)
	build.Pragma("noescape")
	build.Doc(fmt.Sprintf("%s divides x1 by x2, element-wise, storing the result in y (%d bits, SSE required).", name, bits))
